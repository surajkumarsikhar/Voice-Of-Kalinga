...
const Video = () => {
  ...
  useGSAP(() => {
    ...
    ScrollTrigger.create({
      ...
      pin: true,
      scrub: true,
      // Keep pinSpacing true to maintain layout
    });
  ...
  return (
    <div ref={containerRef} className="relative h-screen w-full overflow-hidden" id="page">
      {/* Background Video */}
      ...
      <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-6 sm:gap-6 px-4 text-center pt-16 sm:pt-0">
        ...
        <a
          id="nominate-btn"
          href="/nominate"
          className="mt-4 sm:mt-6 px-6 py-2 sm:py-3 sm:px-8 rounded-full border border-white text-white font-semibold text-base sm:text-lg hover:bg-white hover:text-black scale-100 hover:scale-105 transition-all duration-200"
        >
          Nominate a Hero
        </a>
      </div>
